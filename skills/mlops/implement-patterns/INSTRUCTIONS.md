...
**Load** `../assess-mlops/references/mlops-pattern-framework.md` if maturity context is needed.
...
**If routed from assess-mlops with a roadmap**, use the known promotion pattern, maturity levels, and environment names. Skip to Step 3.
...
4. **Environment setup**: How many environments, what names, fully isolated or shared components?
   ...
   Use the user's chosen names, environment count, and isolation model in **all outputs** (checklists, diagrams, recommendations). For full environment guidance (2-env vs 3-env trade-offs, isolation models, canonical name table), see `../assess-mlops/SKILL.md` Step 2.
...
> 1. **Quick path**: Walk them through the decision tree in `../assess-mlops/references/mlops-pattern-framework.md` § "Decision Tree" — this takes ~5 minutes and yields a clear pattern choice.
> 2. **Full path**: Recommend a full assessment via `../assess-mlops/SKILL.md` for comprehensive maturity + pattern evaluation (~15 minutes).
...
If maturity level is unknown, estimate based on their answers or suggest running `../assess-mlops/SKILL.md` first. **Never ask the user to self-assess their maturity level without first explaining what each level means.**
...
- Suggest running `../assess-mlops/SKILL.md` first for a full assessment, or estimate based on their answers.
