"""Tests for the safe_apis.json / kb_rules.json reconciliation fix — Java parity.

Mirrors test_safe_apis_kb_reconciliation.py for the Java analyzer.
...
def _block(code: str, functions: list[str]) -> A.JavaCodeBlock:
    return A.JavaCodeBlock(code=code, line_start=1, line_end=1, block_type="expr",
                           functions=functions)
...
def test_process_single_block_surfaces_groupby_divergence_after_reconciliation():
    ...
    rdd_result, block_data = A._process_single_block(
        block, rag, Path("test.java"), 0.55, reconciled
    )
...
def test_process_single_block_pre_reconciliation_silently_drops_groupby():
    ...
    rdd_result, block_data = A._process_single_block(
        block, _ExplodingRAG(), Path("test.java"), 0.55, safe_apis
    )
...
def test_process_single_block_genuinely_safe_block_still_skips_rag():
    reconciled = reconcile_safe_apis(load_safe_apis(), load_kb_anchor_leaves())
    block = _block('df.select("a").filter(df.col("a").gt(1))', ["select", "filter"])

    rdd_result, block_data = A._process_single_block(
        block, _ExplodingRAG(), Path("test.java"), 0.55, reconciled
    )
