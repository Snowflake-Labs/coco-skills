...
Spark/Scala/Delta ecosystem, or a Spark-adjacent library (Azure Synapse utilities,
...
After Phase 1.1a runs, any Spark-relevant unknown APIs are also present as
`needs_adjudication` rows. **Skip this phase only** when a workload has no
`needs_adjudication` rows.
